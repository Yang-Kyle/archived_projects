...
int lcd_draw_main()
{
    lcd_draw_jpeg(0, 0, "background", NULL, 0, 1);
    lcd_draw_jpeg(0, 0, "bar", NULL, 0, 1);
    
    for (int i = 0; i < 12; i++) {
        lcd_draw_jpeg(10+i*65, 47, "key", NULL, 0, 1);
    }

    lcd_draw_jpeg(230, 404, "logo", NULL, 0, 1);
...
int main(void)
{
   ...
   /* 1 >= i && i < 12 */
     ...
     while (1) {
        ...
        if(i<12){
        new_flag = i;
        if(old_flag==0 && new_flag == 0){lcd_draw_jpeg(10+65*old_flag, 47, "key", NULL, 0, 1);}
        if (new_flag != old_flag) {
            lcd_draw_jpeg(10+65*old_flag, 47, "key", NULL, 0, 1);
        }

        if (x >= 10 && x < 790 && y >= 47 && y < 327) {
            lcd_draw_jpeg(10+65*new_flag, 47, "key_on", NULL, 0, 1);   
            pthread_create(&music_tid, NULL, play_music, (void*)&new_flag);
        }
